...
<template>
  <section class="py-20 px-4 bg-gradient-to-br from-gray-900 via-emerald-950 to-gray-900">
    <div class="max-w-4xl mx-auto text-center">
      ...
      <h2 class="text-3xl md:text-4xl font-bold text-white mb-4">{{ t('newsletterTitle') }}</h2>
      <p class="text-gray-400 mb-8 max-w-md mx-auto">{{ t('newsletterDesc') }}</p>
      ...
      <div class="max-w-md mx-auto">
        <div v-if="status === 'success'" class="flex items-center justify-center gap-2 text-emerald-400 py-4">
          ...
          <span class="font-medium">{{ t('subscribedSuccess') }}</span>
        </div>
        ...
        <p class="text-xs text-gray-500 mt-3">{{ t('noSpam') }}</p>
      </div>
    </div>
  </section>
</template>
